    ...
    private static void play(LinkedList<Song>playlist){
        ...
        while(!quit){
            ...
            switch(action){
                ...
                case 6:
                    if(playlist.size()>0){
                        listIterator.remove(); //Can only be made once per call of next() or previous()
                                               //Will throw an exception if neither has been called.
                                               //Once a record is removed -> A call to either next or previous must be made

                        //Let's guard against this exception
                        if(listIterator.hasNext()){
